    ...
    describe('getShots', function () {
        ...
        it('should not scan paths in .errorshots whom don\'t contain a wildcard', () => {
            ...
            findSpy.firstCall.args.pop()
                ...
                .forEach(findArg => {
                    expect(wildcardedPaths).to.contain(findArg);
                    expect(nonWildcardPaths).to.not.contain(findArg);
                });
        });

        it('should not scan any paths when no wildcard pattern exists in .errorshots', () => {
            const nonWildcardPaths = ['this-is-a-folder'];
            ...
            const { getShots } = proxyQuire('./list', Object.assign(mockSpawnSync({
                ...
                fs: {
                    existsSync: () => true,
                    readFileSync: () => [...nonWildcardPaths].join('\n'),
                }
            }));
